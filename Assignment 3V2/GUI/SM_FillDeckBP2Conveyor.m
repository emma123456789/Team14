%SM_FillDeckBP2Conveyor adds the appropriate commands to the send queue to move a
%block from a table BP to the conveyor
%
%X1, Y1 indicate the position of the block on the table
%X2, Y2 indicate the position to move the block to on the conveyor
%
...
%3. Move slowly down to the block position on the table
...
%7. Move slowly down to conveyor
...
function SM_FillDeckBP2Conveyor(X1, Y1, X2, Y2)
    global queue;
    fast = 'v500';
    regular = 'v100';
    slow = 'v50';
    ...
    commandStr = sprintf('moveerb %.3f %.3f %.3f %.3f %.3f %.3f %s', X2,Y2,conveyor_grip_height,rot,180,0,regular);
    queue.add(commandStr);
